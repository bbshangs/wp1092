...
const tidyUpData = (data, result) => {
  ...
  for (let i = 0; i < data.length; i++)
    station_types += data[i].station_type
  // console.log(station_types)
  const type = new Set(station_types)
  // console.log("type = ", type)
  for (let t of type) {
    const d = []
    for (let i = 0; i < data.length; i++) {
      if (data[i].station_type === t)
        d.push(data[i])
    }
    d.sort((a, b) => a.station_order - b.station_order)
    result[t]=d
  }
...
const GetStations = async (req, res) => {
  ...
  try {
    ...
    // const d = await Station.find({ 'station_type' : 'R'})
    // console.log(d)
